...
namespace nxs{namespace network
{
    ...
    basic_connexion<IO_Type>::basic_connexion(asio::io_service& ios, std::unique_ptr<network::protocol> p) :
    ...
    void basic_connexion<IO_Type>::read()
    {
        ...
        _socket.async_read_some(asio::buffer(_buffer.address(), _buffer.capacity()), socket_read);
    }
    ...
    void basic_connexion<IO_Type>::send()
    {
        ...
        asio::async_write(_socket, asio::buffer(data.ptr(), data.size()), progress, data_complete);
    }
    ...
    void basic_connexion<IO_Type>::close(const network::error_code& err)
    {
        if (_socket.is_open())
        {
            ...
            if (_alive) _socket.shutdown(asio::ip::tcp::socket::shutdown_both);
            _socket.close();
            _alive = false;
        }

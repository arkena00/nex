...
{
    try {
    string str_data = string(buf.data(), buf.size());
    if (str_data.find("/favicon.ico") != string::npos)
    {
    ...
    string str_request = str_data.substr(s, e - s) + ";";
...
void http::output_send(const request& req)
{
    ...
    "Content-Type:" + type + "\r\n" +
    header_file +
    "Content-Length:" + to_string(output_data.size()) +
    "\r\n\r\n";
    ...
    {
        std::ifstream file(output_data.get<std::string>(), ios::binary | ios::in);
